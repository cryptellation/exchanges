// A generated module for Exchanges functions
//
...
func (ci *Exchanges) Lint(sourceDir *dagger.Directory) *dagger.Container {
	c := dag.Container().
...
func (ci *Exchanges) LintDagger(sourceDir *dagger.Directory) *dagger.Container {
	c := dag.Container().
		From("golangci/golangci-lint:v1.62.0").
		WithMountedCache("/root/.cache/golangci-lint", dag.CacheVolume("golangci-lint"))
...
func (ci *Exchanges) UnitTests(sourceDir *dagger.Directory) *dagger.Container {
	c := dag.Container().From("golang:" + goVersion() + "-alpine")
	return ci.withGoCodeAndCacheAsWorkDirectory(c, sourceDir).
...
func (ci *Exchanges) dbIntegrationTests(sourceDir *dagger.Directory) *dagger.Container {
...
func (ci *Exchanges) binanceIntegrationTests(
	sourceDir *dagger.Directory,
...
func (ci *Exchanges) IntegrationTests(
	sourceDir *dagger.Directory,
...
// EndToEndTests runs the end-to-end tests with all required services (DB, Temporal, Exchanges) and env variables.
func (ci *Exchanges) EndToEndTests(
	sourceDir *dagger.Directory,
	binanceApiKey *dagger.Secret, //nolint:revive,stylecheck
	binanceSecretKey *dagger.Secret,
) *dagger.Container {
	...
	exchanges := Runner(dag, sourceDir, temporal, db, binanceApiKey, binanceSecretKey)
...
func (ci *Exchanges) PublishContainer(
	ctx context.Context,
	sourceDir *dagger.Directory,
) error {
	// Create Git repo access
...
func (ci *Exchanges) withGoCodeAndCacheAsWorkDirectory(
	c *dagger.Container,
	sourceDir *dagger.Directory,
) *dagger.Container {
	containerPath := "/go/src/github.com/cryptellation/exchanges"
	return c.
		// Add Go caches
