...
func (e Exchange) Merge(e2 Exchange) Exchange {
	return Exchange{
		Name:    e.Name,
		Periods: mergeStringSlicesIntoUnique(e.Periods, e2.Periods),
		Pairs:   mergeStringSlicesIntoUnique(e.Pairs, e2.Pairs),
		Fees:    e.Fees,
	}
}
...
func (e *Exchange) AddPair(symbols ...string) {
	e.Pairs = mergeStringSlicesIntoUnique(e.Pairs, symbols)
}

// AddPeriods adds a period to the exchange.
func (e *Exchange) AddPeriods(symbols ...string) {
	e.Periods = mergeStringSlicesIntoUnique(e.Periods, symbols)
}
...
func mergeStringSlicesIntoUnique(slices ...[]string) []string {
	// Create a map for the result
	m := make(map[string]bool, 0)

	// Merge the slices into the map
